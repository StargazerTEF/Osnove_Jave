...
//Placeholder je označeno mesto u stringu na kom treba da se postavi prava vrednost iz niza
//vrednosti. Placeholderi su označeni kombinacijom `%` i pozicijom za zamenu.
//Na primer, ukoliko SQL upit izgleda ovako:
...
//Ovaj program koristi `replace` metodu da zameni placeholder pozicije u SQL upitu sa
//stvarnim vrednostima iz niza.
//Objasnjanje:
...
//Napomena: Smatrajte da se broj placeholdera podudara sa brojem elemenata u nizu.
        ...
        userInfo.add("Cardinal");
        userInfo.add("Tom B. Erichsen");
        userInfo.add("Skagen 21");
        userInfo.add("Stavanger");
        userInfo.add("4006");
        userInfo.add("Norway");

        for (int i = 0; i < userInfo.size(); i++) {
            sqlUpit = sqlUpit.replace("%" + i, userInfo.get(i));
        }
